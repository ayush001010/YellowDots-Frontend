...
const youtubeShortsStudioRigu = [
  'FZXiWHJtzQo',
  'Bco2USB48uY',
  '7kRYi1LKQDU',
];

const youtubeShortsVrinda = [
  'zPLiaQ-C-V8',
  'wnYtfYzAqwA',
  '-RFGMum_V3s',
  's_r3bw-jc5A',
...
const VideoThumbnailPlayer = ({ videoId }) => {
  ...
  return (
    ...
    >
      {/* Static container to fix sizing and prevent flicker */}
...
const Section = ({ img, alt, text, reverse = false }) => (
  <div
    ...
    } items-center md:items-start gap-6 py-12`}
  >
    <img
      src={img}
      alt={alt}
      className="w-[30vh] aspect-square object-cover rounded-full shadow-md"
    />
    <p
      className="text-base font-normal leading-normal py-3"
      dangerouslySetInnerHTML={{ __html: text }}
    />
  </div>
);

const renderGrid = (videoIds = []) => (
  <div className="flex overflow-x-auto gap-4 no-scrollbar scroll-smooth scroll-pl-4 snap-x snap-mandatory py-6">
    {videoIds.map((id, index) => (
      <VideoThumbnailPlayer key={index} videoId={id} />
    ))}
  </div>
);
    ...
    <div className="bg-black text-white">
      ...
      <div className="px-4 sm:px-[15%] pt-20 pb-10 border-b border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
        <Section
          ...
          text="<strong>Bahar Dhawan Rohtagi</strong> is a distinguished Indian contemporary artist renowned for her innovative mixed-media creations that seamlessly blend her legal background with her artistic pursuits. Initially practicing law for five years, Bahaar transitioned to art, becoming a self-taught artist whose works are deeply influenced by cosmic themes, dreams, and societal narratives. Her art employs diverse materials such as resin, rubber, cement, and sand, resulting in textured pieces that explore themes like feminism, ecology, and the cosmos. Bahaar's unique approach has garnered international recognition, including the prestigious SAARC Award for Best Emerging Artist in 2017. <br /><br />Beyond her artistic endeavors, Bahaar is also celebrated as an influencer and advocate for sustainability in art. Her journey from law to art exemplifies a fusion of discipline and creativity, making her a prominent figure in India's contemporary art scene."
        />
        {renderGrid(youtubeShortsBahar)}
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 w-full">
          <div className="md:w-1/3 text-center md:text-left">
            <img
              src={studioRiguLogo}
              alt="Studio Rigu"
              className="w-[35vh] aspect-square object-center rounded-full shadow-md"
            />
          ...
          <div className="w-full md:w-2/3">{renderGrid(youtubeShortsStudioRigu)}</div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-[15%] py-12 border-b border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
        ...
        />
        {renderGrid(youtubeShortsVrinda)}
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 w-full">
          <div className="md:w-1/3 text-center md:text-left">
            <img
              src={nonames}
              alt="No Names"
              className="w-[35vh] aspect-square object-cover rounded-full shadow-md"
            />
            <p className="mt-4 text-base font-normal leading-normal">
          ...
          <div className="w-full md:w-2/3">{renderGrid(youtubeShortsNoNames)}</div>
        </div>
      </div>
    </div>
  );
